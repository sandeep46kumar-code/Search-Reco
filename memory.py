...
def get_recent_recommendations(limit: int = 50, status: str = None) -> list[dict]:
    sql = "SELECT * FROM recommendations"
    params = []
    if status == "approved":
        sql += " WHERE gatekeeper = 'APPROVED'"
    elif status == "rejected":
        sql += " WHERE gatekeeper = 'REJECTED'"
    sql += " ORDER BY run_date DESC LIMIT %s"
    params.append(limit)
    with get_conn() as conn:
        with conn.cursor() as cur:
