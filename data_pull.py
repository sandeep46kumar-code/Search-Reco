...
def pull_campaign_data(customer_id: str, days: int = 14,
                        start_date=None, end_date=None) -> pd.DataFrame:
    """
    Pull campaign-level metrics.
    If start_date/end_date provided, uses those exact dates (for primary/recency windows).
    Otherwise falls back to last `days` days.
...
def _pull_from_api(customer_id: str, days: int,
                   start_date=None, end_date=None) -> pd.DataFrame:
