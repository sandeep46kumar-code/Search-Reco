...
def run_pipeline():
    ...
    attribution_n = int(body.get("attribution_n") or os.getenv("ATTRIBUTION_WINDOW_DAYS", 14))
    with _run_lock:
...
@app.route("/attribution-config", methods=["GET"])
def get_attribution_config():
    """Return current attribution window config and computed date ranges."""
    from stability import get_attribution_windows
    n = int(os.getenv("ATTRIBUTION_WINDOW_DAYS", 14))
    windows = get_attribution_windows(n)
    return ok({
        "attribution_n":    n,
        "windows":          windows,
        "recency_weight":   0.30,
        "note":             "Primary window used for CPA. Last N days used at 30% weight for trend direction only.",
    })
